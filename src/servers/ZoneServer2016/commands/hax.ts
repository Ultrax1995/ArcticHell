...
const hax: any = {
    ...
    switch (args[1]) {
      ...
      default:
        if (args.length < 4) {
          ...
          server.sendChatText(
            client,
            "Set location list: farm, zimms, pv, br, ranchito, drylake, dam, cranberry, church, desoto, toxic, radiotower, villas, hospital",
            false
          );
          return;
        }
  ...
  run: function (server: ZoneServer2016, client: Client, args: any[]) {
	...
	server.sendAlertToAll(`Broadcast: ` + client.character.name + ` set his run speed to ${args[1]} `);
  },
  ...
  build: function (server: ZoneServer2016, client: Client, args: any[]) {
	...
	server.sendChatText(client, "Remember: Don't Build Close To PVP-ZONE");
