...
import { _, Int64String, isPosInRadius, getDistance, toInt, eul2quat, quat2matrix, isInside } from "../../utils/utils";
  ...
  constructor() {
    ...
    this.ClientFinishedLoading = function (
      ...
      if (client.firstLoading) {
        ...
        setTimeout(() => {
          server.sendAlert(client, `Arctic Hell server - Do not build in PVP Zone!`);
          server.sendChatText(client, "Sniper BattleRoyale ! :D", true);
          server.sendChatText(
            client,
            `Players Online : ${_.size(server._characters)}`
          );
          if (client.isAdmin) {
